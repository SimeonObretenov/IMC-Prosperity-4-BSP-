...
class Trader:
    def __init__(self):
        # --- Hyperparameters ---
        # Assuming a standard position limit. Adjust this to the exact limit given in the competition rules.
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            if len(order_depth.sell_orders) != 0:
                ...
                # We only care about the best ask because we are aggressively buying
                best_ask = min(order_depth.sell_orders.keys())
                
                current_position = state.position.get(product, 0)
                
                # --- 2. Buy and Hold Execution ---
                # Calculate how much we can buy before hitting the limit
                max_buy_volume = self.POSITION_LIMIT - current_position
                
                if max_buy_volume > 0:
                    # Send a buy order matching the best ask price to guarantee an immediate fill.
                    # This acts as a market order.
                    orders.append(Order(product, best_ask, max_buy_volume))
                    
                # Notice we do NOT append any sell orders. We are just holding.
